...
#define ST7735_DATA_BUF_SIZE 256
...
typedef struct
{
    ...
    uint8_t height;
    uint8_t tabcolor;
    uint8_t buf[ST7735_DATA_BUF_SIZE];
} st7735_t;
...
static void send_command_list(st7735_t* inst, const uint8_t* addr) {
    ...
    while (num_commands--) {                       // For each command...
        ...
        while (num_args--) {                                 //   For each argument...
...
static void push_color_array(st7735_t* inst, int count, uint16_t color) {
    uint8_t color_h = (color >> 8) & 0xff;
    uint8_t color_l = color & 0xff;
    int remain = count;
    uint8_t* buf = inst->buf;
    while (remain > 0) {
        int send_size = (remain > ST7735_DATA_BUF_SIZE / 2) ? (ST7735_DATA_BUF_SIZE / 2) : remain;
...
static void draw_hline(const struct graphic_apis_s* instance, uint8_t x1, uint8_t y1, uint8_t x2, uint16_t color) {
    ...
    int remain = x2 - x1 + 1;
    push_color_array(inst, remain, color);
    inst->spi->spi_release_lock(inst->spi, &(inst->device));
}
...
static void fill_rect_raw(st7735_t* inst, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint16_t color) {
    ...
    push_color_array(inst, area, color);
}
...
static void draw_char(const struct graphic_apis_s* instance, uint8_t x, uint8_t y, uint8_t size, uint8_t c, uint16_t text_color, uint16_t background_color) {
    st7735_t* inst = container_of(instance, st7735_t, graphic_api);
    inst->spi->spi_aquire_lock(inst->spi, &(inst->device));
    size_t font_slice_size = size * (ASCII_FONT_HEIGHT + 1) * size;
    if (font_slice_size > ST7735_DATA_BUF_SIZE / 2) {
        for (int8_t i = 0; i < ASCII_FONT_WIDTH + 1; i++) {
            uint8_t line;
            if (i < ASCII_FONT_WIDTH) {
                line = pgm_read_byte(font + (c * 5) + i);
            } else {
                line = 0x0;
            }

            for (int8_t j = 0; j < ASCII_FONT_HEIGHT + 1; j++, line >>= 1) {
                if (line & 0x1) {
                    uint8_t x1 = x + (i * size);
                    uint8_t y1 = y + (j * size);
    ...
    } else {  //(font_slice_size > ST7735_DATA_BUF_SIZE / 2)
        uint8_t* buf = inst->buf;
        uint8_t text_color_h = (text_color >> 8) & 0xff;
        uint8_t text_color_l = text_color & 0xff;
        uint8_t bg_color_h = (background_color >> 8) & 0xff;
        uint8_t bg_color_l = background_color & 0xff;
        for (int8_t i = 0; i < ASCII_FONT_WIDTH + 1; i++) {
            uint8_t line;
            if (i < ASCII_FONT_WIDTH) {
                line = pgm_read_byte(font + (c * 5) + i);
            ...
            for (int8_t j = 0; j < ASCII_FONT_HEIGHT + 1; j++, line >>= 1) {
                for (size_t sy = 0; sy < size; sy++) {
                    for (size_t sx = 0; sx < size; sx++) {
                        size_t buf_pos = 2 * (j * size * size + size * sy + sx);
                        if (line & 0x01) {
                            buf[buf_pos] = text_color_h;
                            buf[buf_pos + 1] = text_color_l;
                        } else if (background_color != text_color) {
                            buf[buf_pos] = bg_color_h;
                            buf[buf_pos + 1] = bg_color_l;
                        }
                    }
                }
            }
            uint8_t x1 = x + (i * size);
            set_addr_window(inst, x1, y, x1 + size - 1, y + (ASCII_FONT_HEIGHT + 1) * size - 1);
            writedataarray(inst, buf, font_slice_size*2);
        }
    }
    inst->spi->spi_release_lock(inst->spi, &(inst->device));
}
...
static void draw_bmp(const struct graphic_apis_s* instance, uint8_t x1, uint8_t y1, uint8_t w, uint8_t h, const uint16_t* data) {
    ...
    size_t start = 0;
    while (area > 0) {
        size_t send_size = (area > ST7735_DATA_BUF_SIZE / 2) ? (ST7735_DATA_BUF_SIZE / 2) : area;
        for (size_t i = 0; i < send_size; i++) {
            buf[i * 2] = (data[i + start] >> 8) & 0xff;
            buf[i * 2 + 1] = data[i + start] & 0xff;
        }
        writedataarray(inst, buf, send_size);
        start += send_size;
        area -= send_size;
